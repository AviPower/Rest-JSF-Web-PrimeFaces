...
public class ProveedorFacadeREST {
    ...
    public void edit(@PathParam("id") Integer id, Proveedor entity) {
        proveedorbean.edit(entity);
    }

    @DELETE
    @Path("{id}")
    public void remove(@PathParam("id") Integer id) {
        proveedorbean.remove(proveedorbean.find(id));
    }
    ...
    @GET
    @Produces(MediaType.APPLICATION_JSON)
    public List<Proveedor> listar(@QueryParam("inicio") @DefaultValue("0") String inicio, 
            @QueryParam("cantidad") @DefaultValue("10") String cantidad, 
            @QueryParam("orderBy") @DefaultValue("id") String orderBy,
            @QueryParam("orderDir") @DefaultValue("ASC") String orderDir) {
        return proveedorbean.listar(inicio, cantidad, orderBy, orderDir);
    }
    ...
    public List<Proveedor> findRange(@PathParam("from") Integer from, @PathParam("to") Integer to) {
        return proveedorbean.findRange(new int[]{from, to});
    }
    ...
    public String countREST() {
        return String.valueOf(proveedorbean.count());
    }
